...
class Criteria extends React.Component {
	...
	render() {
		...
		for(var year in this.props['yt-data-criteria']['year_month']){
			console.log(min_year>=year);
			if(min_year>=year){
				min_year = year;
			}
			year_items.push(<li key={year}>{year}</li>)
		}
		console.log(min_year);
		for(var month in month_list){
			console.log(month);
			if(this.props['yt-data-criteria']['year_month']['2017'][month] === 1){
				month_items.push(<li className="on" key={month}>{month_list[month]}月</li>)
			}else{
				month_items.push(<li key={month}>{month_list[month]}月</li>)
			}
		}
